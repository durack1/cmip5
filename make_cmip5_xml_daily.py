...
This script builds *.xml files for daily model variables

PJD 11 Jun 2015     - Started from make_cmip5_xml.py and updated input
PJD 11 Jun 2015     - Added usi and vsi variables to seaIce_vars (Detelina requested)
...
#$#def pathToFile(inpath,start_time): #; # Non-parallel version of code for testing
    ...
    for (path,dirs,files) in scandir.walk(inpath,topdown=True):
        ...
        timeExclude = set(['3hr','6hr','fx','mon','monClim','yr'])
    ...
    temporal    = ['day'] ; # For months and fixed fields only
    ...
    seaIce_vars = ['sic','sit','usi','vsi'] ; seaIce_vars.sort()
    ...
    list_vars   = seaIce_vars ; # Create length counter for reporting
    len_vars = len(list_vars)
...
def xmlWrite(inpath,outfile,host_path,cdat_path,start_time,queue1):
    ...
    outfileName = replace(outfileName,'.day.','.da.') ; # Fix day -> da
...
f = open(os.path.join(log_path,"".join([time_format,'_list_daily_outfiles.pickle'])),'w')
...
import os,pickle
picklefile = '/work/cmip5/_logs/150612_161457_list_daily_outfiles.pickle'
f = open(picklefile,'r')
...
if make_xml:
    ...
    #xml_count2 = len(o)
    o = glob.glob("".join([host_path,'*/*/da/*/*.xml']))
    xml_count1 = len(o)
    ...
    #p = call(cmd,stdout=fnull,shell=True)
    #fnull.close()
    cmd = "".join(['rm -rf ',host_path,'*/*/da_new'])
    fnull = open(os.devnull,'w')
    p = call(cmd,stdout=fnull,shell=True)
    fnull.close()
    ...
    if xmlGood > 1:
        ...
        #p = call(cmd,stdout=fnull,shell=True)
        #fnull.close()
        cmd = 'find */*/da_new -maxdepth 0 -exec sh -c \'mv -f `echo {}` `echo {} | sed s/da_new/da/`\' \;'
        fnull = open(os.devnull,'w')
        p = call(cmd,stdout=fnull,shell=True)
        fnull.close()
    ...
    print "** make_cmip5_xml_daily.py run in report mode **"
    writeToLog(logfile,"** make_cmip5_xml_daily.py run in report mode **")
