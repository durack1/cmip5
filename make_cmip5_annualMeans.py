...
parser.add_argument('realm',nargs='?',default='',metavar='str',type=str,help='including the optional argument \'realm\' will further subprocess variables')
...
if (args.experiment in ['all','1pctCO2','abrupt4xCO2','amip','historical','historicalExt','historicalGHG','historicalNat','piControl','rcp26','rcp45','rcp60','rcp85']):
    ...
    if (args.realm in ['','all','atm','land','ocn','seaIce']):
        ...
        if ( realm == '' or realm == 'all' ) and experiment == 'all':
            ...
            all_realms = True
            realm = 'all' ; # for logfile handling
            print "".join(['** Processing *.xml files from ',experiment,' and all realms for *an.nc file generation **'])
        elif realm != '' and experiment == 'all':
            all_experiments = True
            print "".join(['** Processing *.xml files from ALL experiments and ',realm,' for *an.nc file generation **'])
        elif realm != '':
            print "".join(['** Processing *.xml files from ',experiment,' and ',realm,' for *an.nc file generation **'])
    else:
...
if host_name in {'crunchy.llnl.gov','oceanonly.llnl.gov'}:
    ...
    xml_path   = '/work/cmip5/' ; # oceanonly 130318
    xml_count  = 6
    #xml_path    = '/work/durack1/Shared/cmip5/tmp/' ; # crunchy 130228
    #xml_count   = 9
...
vars_include_atm    = ['pr','prw','ps','tas','ts'] ; # 'evspsbl','hfls','hfss','tauu','tauv','uas','vas'
vars_include_fx     = []
